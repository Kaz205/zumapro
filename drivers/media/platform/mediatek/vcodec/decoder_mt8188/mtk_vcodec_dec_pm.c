...
int mtk_vcodec_init_dec_clk_mt8188(struct platform_device *pdev, struct mtk_vcodec_pm *pm)
{
...
EXPORT_SYMBOL_GPL(mtk_vcodec_init_dec_clk_mt8188);
...
void mtk_vcodec_dec_enable_hardware_mt8188(struct mtk_vcodec_dec_ctx *ctx, int hw_idx)
{
...
EXPORT_SYMBOL_GPL(mtk_vcodec_dec_enable_hardware_mt8188);

void mtk_vcodec_dec_disable_hardware_mt8188(struct mtk_vcodec_dec_ctx *ctx, int hw_idx)
{
	if (IS_VDEC_INNER_RACING(ctx->dev->dec_capability))
...
EXPORT_SYMBOL_GPL(mtk_vcodec_dec_disable_hardware_mt8188);
